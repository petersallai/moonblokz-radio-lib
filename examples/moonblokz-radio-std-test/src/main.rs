...
async fn run(node_id: u32, radio_communication_manager: &'static RadioCommunicationManager) -> ! {
    ...
    loop {
        log!(
            log::Level::Debug,
            "sending message from node {}: {}",
            node_id,
            i
        );
        let payload: [u8; 1111] = [i; 1111];
        let _ = radio_communication_manager.send_message(RadioMessage::add_transaction_with(
            node_id, i as u32, 1, &payload,
        ));
...
async fn main(spawner: Spawner) {
    ...
    _ = radio_communication_manager_temp_1.initialize(
        radio_configuration,
        &spawner,
        radio_device_1,
        0,
    ...
    let radio_communication_manager_1: &'static RadioCommunicationManager =
        Box::leak(Box::new(radio_communication_manager_temp_1));
    log!(
        log::Level::Debug,
        "spawning task to send messages to radio communication manager 1"
    );

    spawner
        .spawn(run(1, &radio_communication_manager_1))
        .unwrap();
    ...
    let res = radio_communication_manager_temp_2.initialize(
        radio_configuration,
        &spawner,
        radio_device_2,
        1,
    ...
    if res.is_err() {
        log!(
            log::Level::Error,
            "Error initializing radio communication manager 2: {:?}",
            res.err()
        );
    }
    log!(log::Level::Debug, "radio communication manager 2 started");
    let radio_communication_manager_2: &'static RadioCommunicationManager =
        Box::leak(Box::new(radio_communication_manager_temp_2));
    log!(
        log::Level::Debug,
        "spawning task to send messages to radio communication manager 2"
    );

    spawner
        .spawn(run(2, &radio_communication_manager_2))
        .unwrap();
    ...
    loop {
        let (message, _) = select_array([
            radio_communication_manager_1.receive_message(),
            radio_communication_manager_2.receive_message(),
        ])
        .await;
